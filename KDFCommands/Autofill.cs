...
class Autofill {
	...
	public Autofill(Ts3Client ts3Client, PlayManager playManager, PlaylistManager playlistManager, TsFullClient ts3FullClient) {
		Ts3Client = ts3Client;
	...
	public void Disable(Uid uid) {
		...
		Disable();
		Ts3Client.SendChannelMessage("[" + KDFCommands.GetClientNameFromUid(Ts3FullClient, uid) + "] " + Status("now"));
	}
